...
describe('ariaOverload', () => {
  describe('redundant aria-label', () => {
    it('fails when aria-label repeats the element role name', () => {
      const doc = createDoc(
        '<section aria-label="Section"><h2>Content</h2></section>',
      );
      const result = ariaOverload.validate(doc);
      expect(result.passed).toBe(false);
      expect(result.details).toContain('merely repeats');
    });

    it('fails for blockquote with aria-label="Blockquote"', () => {
      const doc = createDoc(
    ...
    it('passes when aria-label provides a descriptive name', () => {
      const doc = createDoc(
  ...
  describe('incorrect roles', () => {
    it('fails when role="menuitem" is used outside a menu', () => {
    ...
    it('passes when role="menuitem" is inside a menu', () => {
      const doc = createDoc(
    ...
    it('fails for link with role="menuitem" outside menu context', () => {
      const doc = createDoc(
  ...
  describe('broken aria-controls', () => {
    it('fails when aria-controls references a non-existent ID', () => {
    ...
    it('passes when aria-controls references an existing ID', () => {
      const doc = createDoc(
  ...
  describe('combined scenarios', () => {
    it('fails on the starter HTML with multiple issues', () => {
      const doc = createDoc(`
    ...
    it('passes on clean solution HTML', () => {
      const doc = createDoc(`
