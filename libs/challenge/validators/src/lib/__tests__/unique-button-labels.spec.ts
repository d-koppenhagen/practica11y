...
describe('uniqueButtonLabels', () => {
  it('passes when all buttons have unique accessible names', async () => {
    const doc = createDoc(`
    ...
    `);
    const result = await uniqueButtonLabels.validate(doc);
    expect(result.passed).toBe(true);
    expect(result.message).toBe('All buttons have unique accessible names.');
  });

  it('fails when multiple buttons share the same text content', async () => {
    const doc = createDoc(`
      <button>Edit</button>
      <button>Edit</button>
  ...
  it('passes when a single button exists (no duplicates possible)', async () => {
    const doc = createDoc(`<button>Submit</button>`);
    const result = await uniqueButtonLabels.validate(doc);
    expect(result.passed).toBe(true);
  });

  it('passes when no buttons exist', async () => {
    const doc = createDoc(`<p>No buttons here</p>`);
    const result = await uniqueButtonLabels.validate(doc);
    expect(result.passed).toBe(true);
  });

  it('uses aria-label for name comparison', async () => {
    const doc = createDoc(`
  ...
  it('uses aria-labelledby for name comparison', async () => {
    const doc = createDoc(`
  ...
  it('fails when aria-labelledby resolves to the same name', async () => {
    const doc = createDoc(`
  ...
  it('treats comparison as case-insensitive', async () => {
    const doc = createDoc(`
      <button>Edit</button>
      <button>edit</button>
    `);
  ...
  it('includes visually hidden text in the accessible name', async () => {
    const doc = createDoc(`
  ...
  it('excludes aria-hidden content from the accessible name', async () => {
    const doc = createDoc(`
  ...
  it('skips buttons with empty names (handled by interactive-element-name)', async () => {
    const doc = createDoc(`
      <button></button>
      <button></button>
      <button>Save</button>
  ...
  it('uses title attribute as accessible name', async () => {
    const doc = createDoc(`
