...
describe('sectionAccessibleName', () => {
  it('fails when section has no aria-label or aria-labelledby', async () => {
    const doc = createDoc('<section><h2>Content</h2></section>');
    const result = await sectionAccessibleName.validate(doc);
    expect(result.passed).toBe(false);
    expect(result.details).toContain('role="generic"');
  });

  it('fails for multiple unlabeled sections', async () => {
    const doc = createDoc(
  ...
  it('passes when section has aria-label', async () => {
    const doc = createDoc(
  ...
  it('passes when section has aria-labelledby', async () => {
    const doc = createDoc(
  ...
  it('passes when no sections are present', async () => {
    const doc = createDoc('<div><h2>Content</h2></div>');
    const result = await sectionAccessibleName.validate(doc);
    expect(result.passed).toBe(true);
  });

  it('reports only unlabeled sections in a mixed set', async () => {
    const doc = createDoc(
